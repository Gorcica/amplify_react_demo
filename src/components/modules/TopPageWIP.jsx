...
const TopPage = () => {
  ...
  const [targetEvent2, setTargetEvent2] = useState();

  const handleEventClick = (eventClickInfo) => {
    console.log(eventClickInfo);
    setEventInfo(eventClickInfo.event);
    setTargetEvent(eventClickInfo);
    setTargetEvent2(eventClickInfo.jsEvent.target);
  };
  ...
  const handleEventContent = (info) => (<Box id={`${info.event.title}${info.event.start}box`}>
    <Box sx={{display: 'flex', alignItems: 'center'}}>
      <Box sx={{mt: 1, mb: 1, ml: 1, display: 'flex', alignItems: 'center'}}>
        {info.event.extendedProps.tag === "Roulette" && <RouletteIcon /> }
        {info.event.extendedProps.tag === "Deadline" && <DeadlineIcon /> }
      </Box>
      <Typography variant="p" sx={{fontSize: 15,display: "flex", whiteSpace: "pre-line", overflowWrap: 'anywhere', ml:1}}>
        {`${info.event.title}`}
      </Typography>
    </Box>
    {console.log(info.jsEvent)}
    <Popover
        id={`${info.event.title}${info.event.start}popover`}
        open={targetEvent && targetEvent.event.title === info.event.title}
        anchorEl={info.jsEvent ? info.jsEvent.target : 'test'}
        anchorOrigin={{
          vertical: 'bottom',
          horizontal: 'left',
        }}
